...
{
    public class AddInsuranceViewModel : INotifyPropertyChanged, IDataErrorInfo
    {
        private readonly VehicleRegistry _registry;
        private readonly string _regNumber;
        private DateTime? _validUntil;

        public AddInsuranceViewModel(string regNumber)
        {
        ...
        private void Save()
        {
            try
            {
                if (!ValidUntil.HasValue)
                    throw new Exception("Wybierz datę ważności ubezpieczenia.");
                _registry.UpdateInsurance(_regNumber, ValidUntil.Value);
